...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config):
        ...
        self.use_full_dataset = False if config['full_dataset'] is None else config['full_dataset']
    ...
    def split_by_ratio(self, ratios, group_by=None, full_dataset=False):
        ...
        if group_by is None:
            ...
            if full_dataset:
                next_index = [[] for _ in range(len(ratios) + 1)]
            else:
        ...
        if split_mode == "RS":
            ...
            if group_by is None or group_by.lower() == "none":
                ...
                datasets = self.split_by_ratio(
                    split_args["RS"], group_by=self.uid_field, full_dataset=self.use_full_dataset
                )
            else:
                raise NotImplementedError(
