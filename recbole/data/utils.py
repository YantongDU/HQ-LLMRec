...
def data_preparation_with_full_dataset(config, dataset):
    """Split the dataset by :attr:`config['[valid|test]_eval_args']` and create training, validation and test dataloader.
    ...
    if dataloaders is not None:
        ...
        train_dataset, valid_dataset, test_dataset, full_dataset = built_datasets
        train_sampler, valid_sampler, test_sampler, full_sampler = create_samplers(
            config, dataset, built_datasets, use_full_dataset=True
        )
        ...
        full_dataset = get_dataloader(config, "test")(
            config, full_dataset, full_sampler, shuffle=False
        )
        if config["save_dataloaders"]:
            save_split_dataloaders(
                config, dataloaders=(train_data, valid_data, test_data, full_dataset)
            )
    ...
    return train_data, valid_data, test_data, full_dataset
...
def _create_sampler(
    ...
    if use_full_dataset:
        phases = ["train", "valid", "test", "test_full"]
    else:
...
def create_samplers(config, dataset, built_datasets, use_full_dataset=False):
    ...
    if use_full_dataset:
        ...
        full_sampler = full_sampler.set_phase("test_full") if full_sampler else None
        return train_sampler, valid_sampler, test_sampler, full_sampler
    else:
