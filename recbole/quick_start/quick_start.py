...
from recbole.data import (
    create_dataset,
    data_preparation,
    data_preparation_with_full_dataset,
)
...
def read_llm_aug(field2id_token_dict):
    # 定义文件路径
    file_path = '/home/dyt/coldstart/RecBole/dataset/ml-100k/ml_100k_augmented_inter_p1_n1.txt'

    #数据集将id转为token
    uid_token = field2id_token_dict['user_id']
    iid_token = field2id_token_dict['item_id']
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            try:
                ...
                # 再次使用 '::' 分割 value，提取 A 和 B
                A, B = value_part.split('::')
                # 将 key 转换为整数，A 和 B 转换为整数
                key = np.where(uid_token == str(int(key_part)+1))[0][0]
                A = np.where(iid_token == str(int(A)+1))[0][0]
                B = np.where(iid_token == str(int(B)+1))[0][0]
                # 将解析后的数据存入字典
                result_dict[key] = {'1': A, '2': B}
...
def run_recbole(
    ...
    use_full_dataset = False if config['full_dataset'] is None else config['full_dataset']
    ...
    # dataset splitting
    if use_full_dataset:
        train_data, valid_data, test_data, full_data = data_preparation_with_full_dataset(config, dataset)
    else:
    ...
    llm_aug_dict = read_llm_aug(dataset.field2id_token)
    config['llm_aug'] = llm_aug_dict
    ...
    if use_full_dataset:
        test_result = trainer.evaluate(
            full_data, load_best_model=saved, show_progress=config["show_progress"], use_full_dataset=use_full_dataset
        )
    else:
        test_result = trainer.evaluate(
