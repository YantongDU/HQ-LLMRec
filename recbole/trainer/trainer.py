...
class Trainer(AbstractTrainer):
    ...
    @torch.no_grad()
    def evaluate(
        self, eval_data, load_best_model=True, model_file=None, show_progress=False, use_full_dataset=False
    ):
        r"""Evaluate the model based on the eval data.
        ...
        if use_full_dataset:
            predict_top10_items = {}
            uid_token2id = eval_data._dataset.field2id_token['user_id']
            iid_token2id = eval_data._dataset.field2id_token['item_id']
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            if use_full_dataset:
                _, topk_indices = torch.topk(scores, 10, dim=1)
                for user_id, candidate_items in zip(interaction['user_id'].tolist(), topk_indices.tolist()):
                    predict_top10_items[uid_token2id[user_id]] = [iid_token2id[k] for k in candidate_items]
        ...
        if use_full_dataset and not os.path.exists('/home/dyt/coldstart/RecBole/candidate_items_BPR'):
            import pickle
            pickle.dump(predict_top10_items, open('/home/dyt/coldstart/RecBole/candidate_items_BPR', 'wb'))
        return result
...
class NCLTrainer(Trainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            with amp.autocast(enabled=self.enable_amp):
                loss, loss_aug = loss_func(interaction)

            grad_aug = torch.autograd.grad(loss_aug, self.model.parameters(), retain_graph=True)
            grad_org = torch.autograd.grad(loss, self.model.parameters(), retain_graph=True)

            similarity = self.cal_similarity_of_grad(grad_aug, grad_org)

            # Get the indices that would sort the similarity tensor in descending order
            sorted_indices = torch.argsort(similarity, dim=0, descending=True)

            # Select the top 50% of the indices
            top_indices = sorted_indices[:self.filter_ratio * self.batch_size]

            # Gather the corresponding loss values and compute their sum
            selected_loss = grad_aug[top_indices]
            losses = selected_loss.sum()
    ...
    def cal_similarity_of_grad(self, grad1, grad2):
        ...
        for idx, _ in enumerate(grad1):
            similarity.append(self.cosine(grad1[idx].flatten().unsqueeze(0), grad2[idx].flatten().unsqueeze(0))[0])
